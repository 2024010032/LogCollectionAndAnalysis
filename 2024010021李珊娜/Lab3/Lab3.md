...
> 记录：本次使用的 Ubuntu 用户名为 __lsn____。
...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 __192.168.246.128____。
...
> 记录：本机主机名为 __lsn-VMware-Virtual-Platform____（即 4W1R 中 Where 的取值）。
...
| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 9 月 22 日 15:22:06.170784，时区 +08:00（CST） |
| Where 在哪里 | 主机 lsn-VMware-Virtual-Platform|
| Who 谁 |指定的标签 lab3_read 写入；正文中标识本人姓名为李珊娜，学号 2024010021 |
| What 做了什么 |通过 logger 向本机日志系统写入一条 write_test 测试消息 |
| Result 结果如何 |正文写明 result=success，结合 journalctl 与 syslog 两处均能查到同一条记录，印证消息已被保存 |

**用一两句话解释这个事件：**

> 填写：2026 年 9 月 22 日 15:22:06，本人在 lsn-VMware-Virtual-Platform 主机上用 logger 命令写入一条学号为 2024010021、姓名为李珊娜的测试消息，正文标记结果为 success。
...
| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 9 月 22 日 15:21:11.723709，时区 +08:00（CST） |
| Where 在哪里 |主机 lsn-VMware-Virtual-Platform |
| Who 谁 |记录程序为 sshd（进程号 6141）；尝试登录的账号为 lsn，连接来源为 192.168.246.1，端口 60976 |
| What 做了什么 | 从 192.168.246.1 通过 SSH（ssh2）尝试用密码登录本机|
| Result 结果如何 |结果为 Failed password，即这次密码认证失败 |

**用一两句话解释这个事件：**

> 填写：2026 年 9 月 22 日 15:21:11，来自 192.168.246.1 的连接用 lsn 账号尝试 SSH 密码登录 lsn-VMware-Virtual-Platform 主机，这次密码认证失败。
...
> 填写：2026 年 9 月 22 日 15:26:34，软件包管理工具 dpkg 记录了 htop（版本 3.3.0-4build1）已安装的状态。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd-journald 负责收集内核、系统服务和程序的消息，并附带服务、进程、启动周期等信息，保存为二进制 journal；rsyslog 负责按消息类别和级别分流，把部分 syslog 消息写入 /var/log/syslog、/var/log/auth.log 等文本文件。3.2 节中 logger 写入的 lab3_read 消息先被 journald 记录进二进制 journal，同时该消息作为 user.notice 级别的 syslog 消息被转发给 rsyslog，由 rsyslog 写入 /var/log/syslog，所以同一条消息能通过 journalctl 和 grep syslog 两个入口同时查到。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：Failed password 能证明：在该时间、该来源 IP 用指定账号尝试 SSH 密码认证失败，记录中包含账号、来源地址和端口。它不能证明：实际操作者是谁、失败的具体原因（密码错误、账号不存在还是其他）、是否构成入侵或攻击，也不能仅凭一行记录判断是否为暴力破解。
